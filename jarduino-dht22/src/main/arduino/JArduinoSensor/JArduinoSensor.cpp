...
// Constructor /////////////////////////////////////////////////////////////////
JArduinoSensor::JArduinoSensor() {
	state = RCV_WAIT;
	buffer_idx = 0;
}

// Public Methods //////////////////////////////////////////////////////////////
void JArduinoSensor::init_JArduino(uint16_t baud = 9600) {
	// init the serial port
	Serial.begin(baud);
}

void JArduinoSensor::poll_JArduino(void) {	
  uint8_t data;
...
void JArduinoSensor::senddigitalReadResult(uint8_t value) {
...
void JArduinoSensor::sendanalogReadResult(uint16_t value) {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
...
void JArduinoSensor::sendpulseInResult(unsigned long value) {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
   payload[3] = 2; // length of the params
...
void JArduinoSensor::sendinterruptNotification(uint8_t interrupt) {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
   payload[3] = 1; // length of the params
...
void JArduinoSensor::sendeeprom_value(uint8_t value) {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
   payload[3] = 1; // length of the params
...
void JArduinoSensor::sendeeprom_write_ack() {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
   payload[3] = 0; // length of the params
...
void JArduinoSensor::senddigitalSensorReadResult(String value) {
   payload[0] = 0x01; // source addr (not used)
   payload[1] = 0x00; // target addr (not used)
   payload[2] = 0x00; // frame num (not used)
   payload[3] = value.length(); // length of the params
   payload[4] = 52; // command code
   uint8_t dataArray[value.length() + 1];
   value.toCharArray(dataArray, value.length() + 1);
   for(int i=0; i<value.length(); i++){
	   payload[i+5] = dataArray[i];
   }
   // send the message
   sendOutgoingMessage(payload, 16);
}

void JArduinoSensor::sendpinStateNotification(uint8_t pin, uint8_t state) {
	payload[0] = 0x01; // source addr (not used)
	payload[1] = 0x00; // target addr (not used)
	payload[2] = 0x00; // frame num (not used)
	payload[3] = 2; // length of the params
...
// Private Methods /////////////////////////////////////////////////////////////
void JArduinoSensor::parseIncommingMessage(uint8_t data[], uint8_t size) {
  ...
  switch(data[4]) // command code
  {
   ...
   case 51: receivedigitalSensorRead(data[5], data[6], data[7]); break;
   default: break; 
  }
}

void JArduinoSensor::sendOutgoingMessage(uint8_t data[], uint8_t size) {
  uint8_t i = 0;
  // send the start byte
